...
# valid images folder 可用作valid或test
...
output_folder = '/home/aistudio'

train_HR_images = []
train_LR_images = []
for i in range(800):
    image_num = '%04d' % (i+1)  # 四位整数
...
valid_LR_images = []
for i in range(100):
    image_num = '%04d' % (i+801)  # 从801开始写的四位整数
    HR_image = image_num + '.png'
    LR_image = image_num + 'x4.png'
...
Set5_HR_images = []
Set5_LR_images = []
images = os.listdir(test_Set5_HR_folder)  # 按图片名写
num = len(images)
for image in images:
    HR_image = image 
...
Set14_HR_images = []
Set14_LR_images = []
images = os.listdir(test_Set14_HR_folder)  # 按图片名写
num = len(images)
for image in images:
    HR_image = image 
    LR_image = image.split('.')[0] + 'x4.png'
...
B100_HR_images = []
B100_LR_images = []
images = os.listdir(test_B100_HR_folder)  # 按图片名写
num = len(images)
for image in images:
    HR_image = image 
    LR_image = image.split('.')[0] + 'x4.png'
...
Urban100_HR_images = []
Urban100_LR_images = []
images = os.listdir(test_Urban100_HR_folder)  # 按图片名写
num = len(images)
for image in images:
    HR_image = image 
    LR_image = image.split('.')[0] + 'x4.png'
